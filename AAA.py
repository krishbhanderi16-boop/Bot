#🗿 Bot Developer - @DORAA_152
#🔰 Developer Channel - @CODWRITHERE
...
MAIN_OWNER = 8785465683 
SUPER_ADMIN_NAME = "@DORAA_152"
SUPER_ADMIN_LINK = "https://t.me/CODWRITHERE"
SUPER_ADMINS = [8785465683]

BOT_TOKEN = "8848800341:AAFbO7YpNIt_dHy47MkgWyYRNH6Doy3zJOY"
LOG_CHANNEL_ID = -1004328135709
...
class S(StatesGroup):
    ...
    redeem_code = State()
    add_firebase = State()
    add_firebase_file = State()  # 🌟 NEW STATE FOR TXT FILE UPLOAD
    add_owner = State()
    add_admin = State()
    ban_user = State()
    unban_user = State()
...
# 🌟 UPDATED FIREBASE MENU KEYBOARD (ADDED TXT BUTTON)
def fb_menu_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    for fb in fbs:
        ...
        rows.append([
            btn(label[:28], "noop", EMOJI_FIRE, "🔥"),
            btn("ʀᴇᴍᴏᴠᴇ", f"owner:fb:del:{fb['id']}", EMOJI_CROSS, "🗑")
        ])
    rows.append([btn("ʙᴀᴄᴋ", "owner:home", EMOJI_GEAR, "🔙")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def cmd_start_deep(msg: Message, state: FSMContext):
    ...
    if not can_use(uid, d):
        await msg.answer(f"{em(EMOJI_CROSS, '⛔')} <b>Access nahi hai!</b>\n\n@DORAA_152 se approval lein. Sahilxalone.t.me ", parse_mode="HTML")
        return
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    if is_banned(uid, d):
        ...
        return
    if not can_use(uid, d):
        await msg.answer(f"{em(EMOJI_CROSS, '⛔')} <b>Access nahi hai!</b>\n\n@DORAA_152 se approval lein.", parse_mode="HTML")
        return
...
async def owner_fb_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=kb([(f"{sc('cancel')}", "owner:fb:menu")]),
        parse_mode="HTML"
    )
...
# 🌟 NEW FEATURE: ADD BULK FIREBASE VIA TXT FILE HANDLER
...
async def owner_fb_add_file_done(msg: Message, state: FSMContext):
    ...
    for line in lines:
        ...
        # Check duplicate inside current file AND inside main database
